...
- `windows-stage.json`: generated by `verify_airi_windows.ps1` from the exact signed AIRI stage and
  approved local VRM used for acceptance;
...
`windows-stage.json` can be generated only with `-RequireAuthenticode`. Both `airi.exe` and the
Godot sidecar must have `Valid` Authenticode status, a signer certificate, and a timestamp
certificate. The report contains only the application version and generation time, four artifact
SHA-256 digests, SHA-256 certificate fingerprints, and approved public model-license fields. It
contains no local paths, certificate subjects, usernames, or model content. Its model digest and
license must match `integrations/airi-v0.11.3/managed-avatar.json`; all four expected artifact
digests must come from the independently reviewed post-signing candidate, not be calculated inline
from an unreviewed directory. Evidence creation refuses to overwrite an existing file.
...
$evidence = "release-evidence\$tag"
New-Item -ItemType Directory -Force -Path $evidence | Out-Null
.\scripts\verify_airi_windows.ps1 `
  -InstallationPath $signedStagePath `
  -ExpectedExeSha256 $approvedAiriExeSha256 `
  -ExpectedAppAsarSha256 $approvedAppAsarSha256 `
  -ExpectedGodotSha256 $approvedGodotSha256 `
  -ModelPath $approvedModelPath `
  -ExpectedModelSha256 $approvedModelSha256 `
  -RequireAuthenticode `
  -AppVersion $version `
  -EvidenceJson "$evidence\windows-stage.json"
